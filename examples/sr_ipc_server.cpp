...
struct ByteBuffer {
    ...
    // Helper to prepare the final IPC response payload: [app_code][msg_len][msg][data...]
    static grpc::ByteBuffer make_response(int32_t code, const std::string& msg, const std::vector<char>& output_data = {}) {
        ByteBuffer buf;
...
class BinaryReader {
    ...
    size_t remaining_;
public:
    BinaryReader(const void* data, size_t size) : ptr_(static_cast<const char*>(data)), remaining_(size) {}
...
void register_handlers() {
    handlers["CreateQueryContext"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        std::string config_path, context_name;
        if (!reader.read_string(config_path) || !reader.read_string(context_name)) {
            return ByteBuffer::make_response(-1, "Invalid arguments");
        }
        CQueryContext ctx = nullptr;
        CStatus status = CreateQueryContext(config_path.c_str(), context_name.c_str(), &ctx);
        
        ByteBuffer resp;
        resp.append_u64(reinterpret_cast<uint64_t>(ctx));
        return ByteBuffer::make_response(status.error_code, status.error_msg ? status.error_msg : "", resp.data);
    };

    handlers["CreateQueryContextV2"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        std::string config, ctx_name, coll_name;
        if (!reader.read_string(config) || !reader.read_string(ctx_name) || !reader.read_string(coll_name)) {
             return ByteBuffer::make_response(-1, "Invalid arguments");
        }
        CQueryContext ctx = nullptr;
        CStatus status = CreateQueryContextV2(config.c_str(), ctx_name.c_str(), coll_name.c_str(), &ctx);
    ...
    handlers["InitContext"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        return ByteBuffer::make_response(status.error_code, status.error_msg ? status.error_msg : "");
    };
    ...
    handlers["GetCollCtxPrtStat"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        CCtxIndexStatus status = GetCollCtxPrtStat(ctx_name.c_str(), static_cast<int64_t>(part_id));
    ...
    handlers["GetCollPrtStat"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        uint64_t part_id;
        if (!reader.read_string(ctx_name) || !reader.read_string(coll_name) || !reader.read_u64(part_id)) {
            return ByteBuffer::make_response(-1, "Invalid arguments");
        }
        CIndexStatus status = GetCollPrtStat(ctx_name.c_str(), coll_name.c_str(), static_cast<int64_t>(part_id));
    ...
    handlers["GetQueryCtxStat"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        CIndexStatus status = GetQueryCtxStat(reinterpret_cast<CQueryContext>(ptr_val));
    ...
    handlers["AssignPartitions"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        CStatus status = AssignPartitions(ctx_name.c_str(), parts.data(), parts.size());
        return ByteBuffer::make_response(status.error_code, status.error_msg ? status.error_msg : "");
    };
    ...
    handlers["GetAssignedPrts"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        int64_t count = 0;
        CStatus status = GetAssignedPrts(ctx_name.c_str(), coll_name.c_str(), &partitions, &count);
    ...
    handlers["GetShardNum"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        int64_t shard_num = 0;
        CStatus status = GetShardNum(ctx_name.c_str(), coll_name.c_str(), &shard_num);
    ...
    handlers["LoadIndex"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        if (!reader.read_string(ctx_name) || !reader.read_string(coll_name) || !reader.read_string(config)) {
             return ByteBuffer::make_response(-1, "Invalid arguments");
        }
        CStatus status = LoadIndex(ctx_name.c_str(), coll_name.c_str(), config.c_str());
        return ByteBuffer::make_response(status.error_code, status.error_msg ? status.error_msg : "");
    };

    handlers["DeleteCollectionIndex"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        std::string ctx_name, coll_name, config;
        if (!reader.read_string(ctx_name) || !reader.read_string(coll_name) || !reader.read_string(config)) {
             return ByteBuffer::make_response(-1, "Invalid arguments");
        }
        CStatus status = DeleteCollectionIndex(ctx_name.c_str(), coll_name.c_str(), config.c_str());
        return ByteBuffer::make_response(status.error_code, status.error_msg ? status.error_msg : "");
    };
    ...
    handlers["GetCollInfo"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        CSnapshot snapshot;
        CStatus status = GetCollInfo(ctx_name.c_str(), names_ptr.data(), static_cast<int>(count), &snapshot);
    ...
    handlers["UnloadCollections"] = [](BinaryReader& reader) -> grpc::ByteBuffer {
        ...
        CStatus status = UnloadCollections(ctx_name.c_str(), names_ptr.data(), static_cast<int>(count));
        return ByteBuffer::make_response(status.error_code, status.error_msg ? status.error_msg : "");
    };
}
...
int main(int argc, char** argv) {
    ...
    runtime.registry().register_kernel("sr_api_dispatcher", [](const std::string& method, const grpc::ByteBuffer& payload) noexcept -> sr::engine::Expected<grpc::ByteBuffer> {
        try {
            ...
            return ByteBuffer::make_response(-1, std::string("Dispatcher error: ") + e.what());
        } catch (...) {
