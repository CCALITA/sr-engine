...
template <typename T>
struct callable_traits<T, std::enable_if_t<
  std::is_function_v<std::remove_pointer_t<std::remove_cvref_t<T>>> ||
  std::is_member_function_pointer_v<std::remove_cvref_t<T>>>> : function_traits<std::remove_cvref_t<T>> {};
...
using output_tuple_t = std::conditional_t<
  ...
  std::tuple<>,
  std::conditional_t<is_tuple_like_v<T>, std::remove_cvref_t<T>, std::tuple<std::remove_cvref_t<T>>>>;

template <typename T>
using call_arg_type_t = std::conditional_t<is_optional<T>, std::remove_cvref_t<T>, T>;
...
constexpr bool kernel_nothrow_invocable(std::index_sequence<I...>) {
  if constexpr (kernel_traits<Fn>::has_ctx) {
    return std::is_nothrow_invocable_v<
      Fn, RequestContextView&, call_arg_type_t<std::tuple_element_t<I, Tuple>>...>;
  } else {
    return std::is_nothrow_invocable_v<
...
template <typename Fn>
constexpr bool kernel_nothrow_ok() {
...
template <typename Fn>
concept KernelFn =
  KernelCallable<Fn> && kernel_inputs_ok<Fn>() && kernel_outputs_ok<Fn>() && kernel_nothrow_ok<Fn>();
...
auto write_tuple_outputs(OutputValues& outputs, Tuple&& tuple, std::index_sequence<I...>) -> void {
  (outputs.set<port_type_t<std::tuple_element_t<I, std::remove_cvref_t<Tuple>>>>(
...
auto apply_kernel(Fn& fn, RequestContextView& ctx, Tuple&& args) -> decltype(auto) {
  if constexpr (HasCtx) {
    return std::apply(
  ...
  return std::apply(
    [&fn](auto&&... arg) -> decltype(auto) { return std::invoke(fn, std::forward<decltype(arg)>(arg)...); },
    std::forward<Tuple>(args));
}
...
template <typename Arg>
auto append_input(std::vector<PortDesc>& inputs, std::string_view name) -> Expected<void> {
...
template <typename Arg>
auto append_output(std::vector<PortDesc>& outputs, std::string_view name) -> Expected<void> {
...
auto for_each_index(Fn&& fn, std::index_sequence<I...>) -> Expected<void> {
  ...
  ((result = result ? std::forward<Fn>(fn)(std::integral_constant<std::size_t, I>{}) : result), ...);
  return result;
}
...
auto append_inputs(std::vector<PortDesc>& inputs, const std::vector<std::string>& names) -> Expected<void> {
  ...
  return for_each_index<Tuple>(add, std::make_index_sequence<std::tuple_size_v<Tuple>>{});
}
...
auto append_outputs(std::vector<PortDesc>& outputs, const std::vector<std::string>& names) -> Expected<void> {
  auto add = [&outputs, &names](auto index_c) -> Expected<void> {
    constexpr std::size_t index = decltype(index_c)::value;
    return append_output<std::tuple_element_t<index, Tuple>>(outputs, names[index]);
  };
  return for_each_index<Tuple>(add, std::make_index_sequence<std::tuple_size_v<Tuple>>{});
}
