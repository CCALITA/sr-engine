...
auto make_const_slot(const Json &value, TypeId expected) -> Expected<ValueBox> {
  if (expected == TypeId{}) {
...
auto apply_port_names(std::vector<PortDesc> &ports,
                      ...
                      std::string_view node_id, std::string_view label)
    -> Expected<void> {
  if (!names.empty()) {
    if (names.size() != ports.size()) {
      return tl::unexpected(make_error(
          std::format("{} name count mismatch for node: {}", label, node_id)));
    }
  ...
  for (const auto &port : ports) {
    if (port.name_id == NameId{}) {
      return tl::unexpected(make_error(
          std::format("missing {} names for node: {}", label, node_id)));
    }
    if (!seen.insert(port.name_id).second) {
      return tl::unexpected(make_error(
          std::format("duplicate {} name for node: {}", label, node_id)));
    }
  }
  return {};
}
  ...
  auto build_nodes() -> Expected<void> {
    ...
    for (const auto &node_def : graph.nodes) {
      if (node_index.contains(node_def.id)) {
        return tl::unexpected(
            make_error(std::format("duplicate node id: {}", node_def.id)));
      }
      auto factory = registry.find(node_def.kernel);
      if (!factory) {
        return tl::unexpected(make_error(
            std::format("kernel not registered: {}", node_def.kernel)));
      }
      auto spec = (*factory)(node_def.params);
      if (!spec) {
        return tl::unexpected(
            make_error(std::format("kernel factory failed: {}: {}",
                                   node_def.kernel, spec.error().message)));
      }

      NodeBuild build;
      build.id = node_def.id;
      build.kernel = std::move(spec->handle);
      build.signature = std::move(spec->signature);

      if (auto name_result = apply_port_names(
              build.signature.inputs, node_def.input_names, build.id, "input");
          !name_result) {
      ...
      if (auto name_result =
              apply_port_names(build.signature.outputs, node_def.output_names,
                               build.id, "output");
          !name_result) {
        return tl::unexpected(name_result.error());
      }
  ...
  auto bind_inputs() -> Expected<void> {
    for (const auto &binding : graph.bindings) {
      ...
      if (to_it == node_index.end()) {
        return tl::unexpected(make_error(
            std::format("binding to unknown node: {}", binding.to_node)));
      }
      ...
      if (input_it == node.input_port_map.end()) {
        return tl::unexpected(
            make_error(std::format("binding to unknown input port: {}.{}",
                                   binding.to_node, binding.to_port)));
      }
      int input_index = input_it->second;
      if (node.input_bound[static_cast<std::size_t>(input_index)]) {
        return tl::unexpected(
            make_error(std::format("input port bound twice: {}.{}",
                                   binding.to_node, binding.to_port)));
      }

      const auto &input_port =
          node.signature.inputs[static_cast<std::size_t>(input_index)];
      ...
      case BindingKind::NodePort: {
        ...
        if (from_it == node_index.end()) {
          return tl::unexpected(make_error(std::format(
              "binding from unknown node: {}", binding.source.node)));
        }
        ...
        if (out_it == from_node.output_port_map.end()) {
          return tl::unexpected(make_error(
              std::format("binding from unknown output port: {}.{}",
      ...
      case BindingKind::Env: {
        ...
        if (env_it == env_index.end()) {
          ...
          env_requirements.push_back(
              EnvRequirement{binding.source.env_key, input_port.type_id});
        } else {
          index = env_it->second;
      ...
      case BindingKind::Const: {
        auto slot =
            make_const_slot(binding.source.const_value, input_port.type_id);
        if (!slot) {
      ...
      node.inputs[static_cast<std::size_t>(input_index)] =
  ...
  auto fill_missing_inputs() -> Expected<void> {
    for (auto &node : node_builds) {
      for (std::size_t i = 0; i < node.signature.inputs.size(); ++i) {
        ...
        auto name = node.input_name_views.size() > i ? node.input_name_views[i]
                                                     : std::string_view{};
        auto label = name.empty() ? std::to_string(i) : std::string(name);
        if (node.signature.inputs[i].required) {
          return tl::unexpected(make_error(
              std::format("missing required input: {}.{}", node.id, label)));
        }
        return tl::unexpected(make_error(
            std::format("missing input binding: {}.{}", node.id, label)));
      }
    }
    return {};
  }
  ...
  auto bind_outputs(ExecPlan &plan) -> Expected<void> {
    ...
    for (const auto &output : graph.outputs) {
      ...
      if (node_it == node_index.end()) {
        return tl::unexpected(make_error(
            std::format("output from unknown node: {}", output.from_node)));
      }
      ...
      if (out_it == node.output_port_map.end()) {
        return tl::unexpected(
            make_error(std::format("output from unknown port: {}.{}",
                                   output.from_node, output.from_port)));
      }
      if (output.as.empty()) {
      ...
      if (!seen.insert(name_id).second) {
        return tl::unexpected(
            make_error(std::format("duplicate output name: {}", output.as)));
      }
  ...
  auto build_dependents(ExecPlan &plan) -> Expected<void> {
    ...
    for (std::size_t node_index = 0; node_index < plan.nodes.size();
      ...
      for (const auto &binding : node.inputs) {
        ...
        int producer =
            plan.slot_producer[static_cast<std::size_t>(binding.slot_index)];
        if (producer < 0) {
        ...
        plan.dependents[static_cast<std::size_t>(producer)].push_back(
  ...
  auto build_plan(std::vector<int> topo, const CompileOptions &options) -> Expected<ExecPlan> {
    ...
    for (std::size_t node_index = 0; node_index < plan.nodes.size();
      ...
      for (int slot_index : node.outputs) {
        plan.slot_producer[static_cast<std::size_t>(slot_index)] =
            static_cast<int>(node_index);
      }
    }
