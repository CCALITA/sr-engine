...
class KernelRegistry {
 public:
  ...
  auto register_kernel(std::string name, Fn fn) -> void {
    ...
    auto handle = detail::make_kernel_handle_from_fn(std::move(fn), inputs, outputs);
  ...
  auto register_kernel_with_params(std::string name, Factory factory) -> void {
    ...
    register_factory(
      ...
      [factory = std::move(factory), inputs = std::move(inputs),
       outputs = std::move(outputs)](const Json& params) mutable -> Expected<KernelHandle> {
        ...
        if constexpr (detail::is_expected_v<FactoryResult>) {
          ...
          return detail::make_kernel_handle_from_fn(std::move(*fn_result), inputs, outputs);
        } else {
          auto fn = factory(params);
          return detail::make_kernel_handle_from_fn(std::move(fn), inputs, outputs);
        }
      });
  }
