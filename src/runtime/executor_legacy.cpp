...
// Per-run state allocated on each LegacyExecutor::run call.
struct DAGOpState : std::enable_shared_from_this<DAGOpState> {
...
auto DAGOpState::prepare(const ExecPlan &plan_ref, RequestContext &ctx_ref,
                         exec::static_thread_pool &pool_ref) -> Expected<void> {
  plan = &plan_ref;
  ctx = &ctx_ref;
  trace = &ctx_ref.trace;
...
auto DAGOpState::schedule_initial_nodes() -> void {
...
auto DAGOpState::schedule_node(int node_index) -> void {
  if (!plan || !pool) {
    return;
  }
  if (node_index < 0 ||
...
auto DAGOpState::execute_node(int node_index) -> void {
...
auto DAGOpState::complete_node(int node_index) -> void {
...
auto DAGOpState::finish_node() -> void {
  if (pending_nodes.fetch_sub(1, std::memory_order_acq_rel) == 1) {
    done_latch.count_down();
  }
}

auto DAGOpState::record_error(std::string message) -> void {
  bool expected = false;
...
struct LegacyExecutor::Pools {
  explicit Pools(int threads) : pool(static_cast<std::size_t>(threads)) {}
  exec::static_thread_pool pool;
};

LegacyExecutor::LegacyExecutor(ExecutorConfig config) {
...
LegacyExecutor::~LegacyExecutor() = default;

auto LegacyExecutor::run(const ExecPlan &plan, RequestContext &ctx) const
    -> Expected<ExecResult> {
  auto op_state = std::make_shared<DAGOpState>();
  if (auto prepared = op_state->prepare(plan, ctx, pools_->pool); !prepared) {
    return tl::unexpected(prepared.error());
  }

  op_state->schedule_initial_nodes();
  op_state->done_latch.wait();

  auto emit_run_end = [op_state](trace::SpanStatus status) {
    if constexpr (trace::kTraceEnabled) {
      auto *trace_ctx = op_state->trace;
      if (!trace_ctx || op_state->trace_flags == 0) {
        return;
      }
      if (!trace::has_flag(op_state->trace_flags, trace::TraceFlag::RunSpan) ||
          op_state->run_span == 0) {
        return;
      }
      trace::Tick end_ts = trace_ctx->now();
      trace::Tick duration = 0;
      if (op_state->run_start != 0 && end_ts >= op_state->run_start) {
        duration = end_ts - op_state->run_start;
      }
      trace::emit(trace_ctx->sink,
                  trace::RunEnd{op_state->trace_id, op_state->run_span, end_ts,
                                duration, status});
    }
  };

  if (op_state->has_error.load(std::memory_order_acquire)) {
    ...
    emit_run_end(status);
    std::lock_guard<std::mutex> lock(op_state->error_mutex);
    return tl::unexpected(op_state->error);
  }

  emit_run_end(trace::SpanStatus::Ok);
  return collect_outputs(plan, op_state->slots);
}
