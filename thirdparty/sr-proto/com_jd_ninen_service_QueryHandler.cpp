...
enum class ErrorCode { Success = 0, UnexpectedError = 1 };

// RAII for JNI String
class JStringHandleScope {
public:
  JStringHandleScope(JNIEnv *env, jstring &jinput_str)
      : env_(env), jinput_str_(jinput_str) {
...
struct ByteBuffer {
  ...
  void append_float_array(const jfloat *arr, jsize len) {
    append_u32(static_cast<uint32_t>(len));
    if (len > 0) {
      data.insert(data.end(), reinterpret_cast<const char *>(arr),
...
public:
  BinaryReader(const std::vector<char> &data)
      : ptr_(data.data()), remaining_(data.size()) {}
...
IpcResponse Call(const std::string &method, const ByteBuffer &args) {
  ...
  // Write Method
  if (!write_string(fd, method)) {
    ::close(fd);
    return {-1, "Write method failed", {}};
  }

  // Write Metadata (Graph Name)
  if (!write_u32_be(fd, 1)) {
    ::close(fd);
    return {-1, "Write meta count failed", {}};
  }
  if (!write_string(fd, "sr-graph-name") || !write_string(fd, kGraphName)) {
    ::close(fd);
    return {-1, "Write meta failed", {}};
  }

  // Write Payload
  if (!write_u32_be(fd, args.data.size()) ||
      !write_all(fd, args.data.data(), args.data.size())) {
    ::close(fd);
    return {-1, "Write payload failed", {}};
  }
  ...
  if (!read_string(fd, resp_payload_str)) {
    ::close(fd);
    return {-1, "Read payload failed", {}};
  }

  ::close(fd);

  std::vector<char> resp_payload(resp_payload_str.begin(),
                                 resp_payload_str.end());

  // Parse our application-level payload: [i32 error_code] [u32 msg_len] [msg]
  // [outputs...]
  BinaryReader reader(resp_payload);
  uint32_t app_err_code_u32 = 0;
  if (!reader.read_u32(app_err_code_u32))
    return {-1, "Malformed response payload", {}};
...
Java_com_jd_ninen_service_QueryHandler_createQueryCtx__Ljava_lang_String_2Ljava_lang_String_2Ljava_lang_String_2(
    JNIEnv *env, jobject jobj, jstring jconfig_path, jstring jcontext_name,
...
JNIEXPORT jboolean JNICALL Java_com_jd_ninen_service_QueryHandler_initContext(
    JNIEnv *env, jobject jobj, jstring jconfig, jstring jcontext_name) {
...
Java_com_jd_ninen_service_QueryHandler_getQueryCtxStat__Ljava_lang_String_2J(
    JNIEnv *env, jobject jobj, jstring jcontext_name, jlong partition_id) {
  ...
  auto resp = ipc::Call(
      "GetCollCtxPrtStat",
      args); // Assuming this maps to deprecated one too or logic is similar
  if (resp.error_code != 0)
    return 0;
...
JNIEXPORT jint JNICALL Java_com_jd_ninen_service_QueryHandler_getCollCtxPrtStat(
    JNIEnv *env, jobject jobj, jstring jcontext_name, jlong partition_id) {
  JStringHandleScope s1(env, jcontext_name);
  ipc::ByteBuffer args;
  args.append_string(s1.GetStr());
...
JNIEXPORT jint JNICALL Java_com_jd_ninen_service_QueryHandler_getCollPrtStat(
    JNIEnv *env, jobject jobj, jstring jcontext_name, jstring jcollection_name,
    jlong partition_id) {
  JStringHandleScope s1(env, jcontext_name);
...
JNIEXPORT jint JNICALL
Java_com_jd_ninen_service_QueryHandler_getQueryCtxStat__J(
    JNIEnv *env, jobject jobj, jlong jquery_ctx_ptr) {
  ipc::ByteBuffer args;
...
JNIEXPORT jboolean JNICALL Java_com_jd_ninen_service_QueryHandler_applyConfig(
    JNIEnv *env, jobject jobj, jstring jindex_name, jstring jconfig_json_str) {
...
Java_com_jd_ninen_service_QueryHandler_assignPartitions(
    JNIEnv *env, jobject jobj, jstring jcontext_name, jlongArray jpartitions) {
  JStringHandleScope s1(env, jcontext_name);
...
Java_com_jd_ninen_service_QueryHandler_getAssignedPrts(
    JNIEnv *env, jobject jobj, jstring jcontext_name,
    jstring jcollection_name) {
  JStringHandleScope s1(env, jcontext_name);
  JStringHandleScope s2(env, jcollection_name);
  ipc::ByteBuffer args;
...
JNIEXPORT jlong JNICALL Java_com_jd_ninen_service_QueryHandler_getShardNum(
    JNIEnv *env, jobject jobj, jstring jcontext_name,
    jstring jcollection_name) {
  JStringHandleScope s1(env, jcontext_name);
  JStringHandleScope s2(env, jcollection_name);
...
JNIEXPORT jbyteArray JNICALL Java_com_jd_ninen_service_QueryHandler_getSnapshot(
    JNIEnv *env, jobject jobj, jint jlevel) {
  ipc::ByteBuffer args;
  ...
  env->SetByteArrayRegion(
      jarr, 0, snapshot_data.size(),
      reinterpret_cast<const signed char *>(snapshot_data.data()));
  return jarr;
}

JNIEXPORT jboolean JNICALL
Java_com_jd_ninen_service_QueryHandler_updateCollections(JNIEnv *env,
                                                         jobject jobj,
...
JNIEXPORT jint JNICALL Java_com_jd_ninen_service_QueryHandler_loadCollIndex(
    JNIEnv *env, jobject jobj, jstring jcontext_name, jstring jcollection_name,
    jstring jconfig) {
  JStringHandleScope s1(env, jcontext_name);
  JStringHandleScope s2(env, jcollection_name);
...
JNIEXPORT jint JNICALL Java_com_jd_ninen_service_QueryHandler_deleteCollIndex(
    JNIEnv *env, jobject jobj, jstring jcontext_name, jstring jcollection_name,
    jstring jconfig) {
  JStringHandleScope s1(env, jcontext_name);
  JStringHandleScope s2(env, jcollection_name);
...
JNIEXPORT jbyteArray JNICALL Java_com_jd_ninen_service_QueryHandler_getCollInfo(
    JNIEnv *env, jobject jobj, jstring jcontext_name,
  ...
  for (int i = 0; i < length; i++) {
    jstring coll_name =
        (jstring)env->GetObjectArrayElement(jcollection_names, i);
  ...
  env->SetByteArrayRegion(jarr, 0, data.size(),
                          reinterpret_cast<const signed char *>(data.data()));
  return jarr;
}

JNIEXPORT jboolean JNICALL Java_com_jd_ninen_service_QueryHandler_reloadFlags(
    JNIEnv *env, jobject jobj, jstring jconfig) {
  JStringHandleScope s1(env, jconfig);
  ipc::ByteBuffer args;
  args.append_string(s1.GetStr());
...
JNIEXPORT jint JNICALL Java_com_jd_ninen_service_QueryHandler_unloadCollections(
    JNIEnv *env, jobject jobj, jstring jcontext_name,
    jobjectArray jcollection_names) {
  JStringHandleScope s1(env, jcontext_name);
  jsize length = env->GetArrayLength(jcollection_names);
...
Java_com_jd_ninen_service_QueryHandler_search__Ljava_lang_String_2Ljava_lang_String_2Ljava_lang_String_2_3F_3J(
    JNIEnv *env, jobject jobj, jstring jcontext_name, jstring jcollection_name,
    jstring jquery_plan_json_str, jfloatArray jquery_vectors,
    jlongArray jpartitions) {
  JStringHandleScope ctx_name_scope(env, jcontext_name);
  JStringHandleScope coll_name_scope(env, jcollection_name);
  JStringHandleScope jstr_scope(env, jquery_plan_json_str);

  jfloat *query_vectors = env->GetFloatArrayElements(jquery_vectors, NULL);
  jsize query_vectors_num = env->GetArrayLength(jquery_vectors);
  jlong *partitions = env->GetLongArrayElements(jpartitions, NULL);
  jsize partition_num = env->GetArrayLength(jpartitions);

  ipc::ByteBuffer args;
  args.append_string(ctx_name_scope.GetStr());
  args.append_string(coll_name_scope.GetStr());
  args.append_string(jstr_scope.GetStr());
  args.append_float_array(query_vectors, query_vectors_num);
  args.append_long_array(partitions, partition_num);

  env->ReleaseLongArrayElements(jpartitions, partitions, 0);
  env->ReleaseFloatArrayElements(jquery_vectors, query_vectors, 0);

  auto resp = ipc::Call("SearchByFloatArray", args);
  if (resp.error_code != 0) {
    return nullptr;
  }

  jbyteArray jarr = env->NewByteArray(resp.payload.size());
  env->SetByteArrayRegion(
      jarr, 0, resp.payload.size(),
      reinterpret_cast<const signed char *>(resp.payload.data()));
  return jarr;
}
